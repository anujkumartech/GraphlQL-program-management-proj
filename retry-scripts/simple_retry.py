...
def main(trace_id):
    ...
    retry_attempts = 60 # Number of retry attempts after a fault
    ...
    for attempt in range(max_attempts):
        ...
        if not is_successful:
            ...
            for retry in range(retry_attempts):
                ...
                print(f"Retry {retry + 1} for fault detected.")
                is_successful, status = check_api_status()
                if is_successful:
                    ...
                    record_retry_info(trace_id, retry_count, fault_duration, "SIMPLE")
                    break
